...
interface FinixLesson {
  title: string;
  titleAr: string;
}

interface FinixModule {
  slug: string;
  code: string;
  track;
...
const finixQuizTiers = ["bronze", "silver", "gold"] ;

const finixQuizTierLabels = {
...
const FINIX_QUIZ_PASS_PERCENT = 80;

interface FinixCertTier {
  id;
  name: string;
  nameAr: string;
  description: string;
...
interface FinixKpiCriterion {
  id: string;
  label: string;
  labelAr: string;
}

interface FinixKpiGroup {
...
const FINIX_KPI_MAX_SCORE = 10;

function finixKpiBand(percent) {
...
console.log(JSON.stringify({finixTracks, finixModules, finixCertTiers, finixKpiGroups, finixSurveyStages, finixQuizTierLabels, FINIX_QUIZ_PASS_PERCENT, FINIX_KPI_MAX_SCORE}));